...
export default function Home() {
  ...
  return (
    <div className="bg-gray-900 w-full min-h-screen flex flex-col gap-y-5 items-center justify-center p-40">
        <h1 className="text-4xl font-extrabold shadow-2xl uppercase text-zinc-400">Page scraper</h1>
        <form onSubmit={handleSubmit}>
          <input onChange={(e) => setUrl(e.target.value)} placeholder="URL to scrape" className="input p-2 bg-zinc-400 " />
          <button type="submit" className="bg-zinc-400 hover:bg-zinc-500 hover:cursor-pointer font-bold p-2 ml-2">{loading ? "..." : "SCRAPE"}</button>
        </form>
